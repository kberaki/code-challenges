...
const listFoods = (recipe) => {
  ...
  for(let i=0; i<recipe.ingredients.length; i++){
    ...
    let firstResult=recipe.ingredients[i].slice(recipe.ingredients[i]
    .indexOf(" ")+1)
    result.push(firstResult.slice(firstResult.indexOf(" ")+1))
...
const splitFoods = (recipe) => {
  ...
  for(let i=0; i<recipe.ingredients.length; i++){
    ...
    let newVar= recipe.ingredients[i].slice(recipe.ingredients[i])
   
    let secondVar = (newVar.split(" "))
    
    let thirdVar =(secondVar.slice(2,recipe.ingredients.length))
    result.push(thirdVar.join().replace(","," ").replace(","," "))
...
const stepActions = (recipe) => {
  ...
  let newVar=recipe.steps[i].split(" ")
  result.push(newVar[0])
  }
  return result;
}
